import { useState, useEffect } from 'react';
import { initializeGapi, signIn, signOut, isSignedIn } from '../services/gmailService';

const EmailAuth = ({ onAuthChange, clientId }) => {
  const [isAuthenticated, setIsAuthenticated] = useState(false);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState(null);

  useEffect(() => {
    const loadGoogleAPI = () => {
      const script = document.createElement('script');
      script.src = 'https://apis.google.com/js/api.js';
      script.onload = async () => {
        try {
          await initializeGapi();
          const signedIn = isSignedIn();
          setIsAuthenticated(signedIn);
          onAuthChange(signedIn);
        } catch (err) {
          setError('Failed to initialize Google API');
        } finally {
          setIsLoading(false);
        }
      };
      script.onerror = () => {
        setError('Failed to load Google API');
        setIsLoading(false);
      };
      document.head.appendChild(script);
    };

    if (!window.gapi) {
      loadGoogleAPI();
    } else {
      initializeGapi().then(() => {
        const signedIn = isSignedIn();
        setIsAuthenticated(signedIn);
        onAuthChange(signedIn);
        setIsLoading(false);
      });
    }
  }, [onAuthChange]);

  const handleSignIn = async () => {
    if (!clientId || clientId === 'your_client_id_here.apps.googleusercontent.com') {
      setError('Google Client ID not configured. Please add your actual Client ID to the .env file.');
      return;
    }

    try {
      setIsLoading(true);
      setError(null);
      await signIn(clientId);
      setIsAuthenticated(true);
      onAuthChange(true);
    } catch (err) {
      console.error('Sign in failed:', err);
      console.error('Error details:', JSON.stringify(err, null, 2));
      
      let errorMessage = 'Failed to sign in. ';
      if (err.error === 'popup_blocked') {
        errorMessage += 'Popup was blocked. Please allow popups for this site.';
      } else if (err.error === 'redirect_uri_mismatch') {
        errorMessage += 'Redirect URI mismatch. Add http://localhost:5173 to authorized origins in Google Cloud Console.';
      } else if (err.error === 'invalid_client') {
        errorMessage += 'Invalid client ID. Check your Google Cloud Console configuration.';
      } else if (err.details) {
        errorMessage += `${err.details}`;
      } else {
        errorMessage += `Error: ${err.error || err.message || 'Unknown error'}`;
      }
      
      setError(errorMessage);
    } finally {
      setIsLoading(false);
    }
  };

  const handleSignOut = async () => {
    try {
      setIsLoading(true);
      await signOut();
      setIsAuthenticated(false);
      onAuthChange(false);
    } catch (err) {
      console.error('Sign out failed:', err);
      setError('Failed to sign out');
    } finally {
      setIsLoading(false);
    }
  };

  if (isLoading) {
    return (
      <div className="auth-container">
        <div className="auth-loading">
          <div className="spinner"></div>
          <p>Loading Google API...</p>
        </div>
      </div>
    );
  }

  if (error) {
    return (
      <div className="auth-container">
        <div className="auth-error">
          <h3>⚠️ Authentication Error</h3>
          <p>{error}</p>
          <button onClick={() => window.location.reload()}>Retry</button>
        </div>
      </div>
    );
  }

  if (!isAuthenticated) {
    return (
      <div className="auth-container">
        <div className="auth-signin">
          <h2>📧 Connect Your Gmail</h2>
          <p>Sign in with Google to sync your emails and see AI-powered priority rankings</p>
          {!clientId && (
            <div className="setup-warning">
              <p><strong>Setup Required:</strong></p>
              <ol>
                <li>Create a Google Cloud project at <a href="https://console.cloud.google.com" target="_blank">console.cloud.google.com</a></li>
                <li>Enable the Gmail API</li>
                <li>Create OAuth 2.0 credentials</li>
                <li>Add your Client ID to .env as VITE_GOOGLE_CLIENT_ID</li>
              </ol>
            </div>
          )}
          <button 
            onClick={handleSignIn} 
            className="signin-button"
            disabled={!clientId}
          >
            <svg width="18" height="18" viewBox="0 0 24 24">
              <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
              <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
              <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
            </svg>
            Sign in with Google
          </button>
        </div>
      </div>
    );
  }

  return (
    <div className="auth-status">
      <span className="auth-indicator">✅ Connected to Gmail</span>
      <button onClick={handleSignOut} className="signout-button">
        Sign Out
      </button>
    </div>
  );
};

export default EmailAuth;